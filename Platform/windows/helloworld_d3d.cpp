...
// the pointer to the input layout
ID3D11InputLayout       *g_pLayout    = nullptr;
// the pointer to the vertex shader            
ID3D11VertexShader      *g_pVS        = nullptr;
// the pointer to the pixel shader              
...
void SetViewPort()
{
    ...
    ZeroMemory(&d3dvp, sizeof(D3D11_VIEWPORT));

    d3dvp.TopLeftX = 0;
    d3dvp.TopLeftY = 0;
    d3dvp.Width = SCREEN_WIDTH;
    d3dvp.Height = SCREEN_HEIGHT;

    g_pDevcon->RSSetViewports(1, &d3dvp);
    return;
}

//load and prepair the shaders
void InitPipeline()
{
    //load two shaders
    ID3DBlob *VS, *PS;
