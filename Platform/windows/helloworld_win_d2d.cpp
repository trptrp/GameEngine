...
ID2D1SolidColorBrush *pCornflowerBrush = nullptr;
...
HRESULT CreateGraphicResources(HWND hwnd)
{
    ...
    if(pRenderTarget == nullptr)
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::CornflowerBlue), &pCornflowerBrush);
        }
    }
    return hr;
}
	...
	case WM_PAINT:
	    {
			...
			if (SUCCEEDED(hr))
			{
                ...
                pRenderTarget->DrawRectangle(&rectangle2, pCornflowerBrush);
